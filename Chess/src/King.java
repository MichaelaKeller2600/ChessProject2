public class King extends Pieces{
	...
	public boolean canMove(int xMove, int yMove, Board gameBoard) {
		...
		if(gameBoard.getGamePeice(xMove, yMove) == null) {
			if ((xMoves <= 1 || xMoves >= -1) && (yMoves <= 1 || yMoves >= -1))
					return true;
		}
		else if(gameBoard.getGamePeice(xMove, yMove).isOpponent((Pieces)this) && ((xMoves <= 1 || xMoves >= -1) && (yMoves <= 1 || yMoves >= -1)))
			return true;
		System.out.println("You can't move that King, oof!");
		return false;
